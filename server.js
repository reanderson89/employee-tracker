    ...
    inquirer
      ...
      .then((data) => {
        connection.query(
          ...
          [data.firstName, data.lastName, data.role_id, 1],
          (err, res) => {
            if (err) throw err;
            console.log(
              `"${data.firstName} ${data.lastName}" has been added as a manager. \n Employee ID#: ${res.insertId} will be used to associate employees to them.`
                .green
            );
            employeeTracker();
          }
...
const viewEmployee = () => {
  connection.query(
    "SELECT * FROM employee INNER JOIN role ON employee.role_id = role.role_id",
    (err, res) => {
      if (err) throw err;
      console.table(res);
      employeeTracker();
...
const viewEmployeesByManager = () => {
  connection.query("SELECT * FROM employee", (err, res) => {
    ...
    inquirer
      .prompt([
        {
          ...
          message:
            "Please choose the manager you would like to see the employees of.",
      ...
      .then((data) => {
        connection.query(
          "SELECT * FROM employee INNER JOIN role ON employee.role_id = role.role_id WHERE manager_id = ?",
          [data.employeesWithManager],
          (err, res) => {
            if (err) throw err;
            console.table(res);
            employeeTracker();
...
const viewDepartmentBudget = () => {
  connection.query("SELECT * FROM department", (err, res) => {
    ...
    inquirer
      ...
      .then((data) => {
        connection.query(
          "SELECT salary FROM role INNER JOIN department ON department.department_id = role.department_id WHERE name = ?",
          [data.departmentChosen],
          (err, result) => {
            if (err) throw err;
            ...
            displayUsedBudget =
              ...
              displayUsedBudget
                ...
                .replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,");
            console.log(
              `The current utilized budget for this department is ${displayUsedBudget}.`
                .green
            );
            employeeTracker();
          }
    ...
    inquirer
      ...
      .then((data) => {
        ...
        if (data.updateChoice === "Role ID") {
          connection.query(
            "SELECT * FROM employee INNER JOIN role ON employee.role_id = role.role_id WHERE employee_id = ?",
            [empId],
            (err, res) => {
              if (err) throw err;
...
const updateSalaryByRole = () => {
  connection.query("SELECT * FROM role", (err, res) => {
    ...
    inquirer
      ...
      .then((data) => {
        connection.query(
          "UPDATE role SET salary=? WHERE role_id=?",
          [data.updatedSalary, data.selectRole],
          (err, res) => {
            if (err) throw err;
...
const deleteEmployee = () => {
  connection.query("SELECT * FROM employee", (err, res) => {
    ...
    if (res.length < 1) {
      ...
      console.log(
        "Please use the above table to find the corresponding employee.".green
      );
      inquirer
        .prompt([
          {
        ...
        .then((data) => {
          connection.query(
            ...
            (err, res) => {
              if (err) throw err;
              console.log(
                `Employee with ID#: "${data.employeeId}" has been deleted.`
                  .green
              );
              employeeTracker();
            }
